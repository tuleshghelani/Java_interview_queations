...
## 9. Implementing a Custom Annotation Processor

**Theory:**
- Annotation processors are a powerful feature of Java that allows for code generation and validation at compile time
- They work with Java's annotation system to process annotations and generate additional source files
- Key components:
  1. `javax.annotation.processing.Processor` interface
  2. `javax.annotation.processing.AbstractProcessor` class
  3. `javax.lang.model` package for working with Java language elements
  4. Service Provider Interface (SPI) for processor registration
- Annotation processors run during compilation, not at runtime
- They can generate new source files but cannot modify existing ones
...
// First, define our custom annotation
...
// Then, implement the annotation processor
...
public class BuilderProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        // Process each element annotated with @GenerateBuilder
        for (Element element : roundEnv.getElementsAnnotatedWith(GenerateBuilder.class)) {
            ...
            // Cast to TypeElement to access more specific methods
            ...
            // Get the suffix from the annotation
            ...
            try {
                generateBuilderClass(packageName, className, suffix, typeElement);
            } catch (IOException e) {
                processingEnv.getMessager().printMessage(
                    Diagnostic.Kind.ERROR,
        ...
        // Return true to claim the annotations we process
        return true;
    }
    
    private void generateBuilderClass(String packageName, String className, String suffix, 
                                     TypeElement typeElement) throws IOException {
        String builderClassName = className + suffix;
        String qualifiedBuilderName = packageName + "." + builderClassName;
        
        // Create a new Java file
        JavaFileObject builderFile = processingEnv.getFiler().createSourceFile(qualifiedBuilderName, typeElement);
        
        // Get all fields of the class to create builder methods
        Set<VariableElement> fields = getFields(typeElement);
        ...
        try (PrintWriter out = new PrintWriter(builderFile.openWriter())) {
            ...
            out.println("/**");
            out.println(" * Builder class for {@link " + className + "}.");
            out.println(" * Generated by BuilderProcessor.");
            out.println(" */");
            ...
            for (VariableElement field : fields) {
                ...
                out.println("    /**");
                out.println("     * Sets the " + fieldName + " property.");
                out.println("     * @param " + fieldName + " the value to set");
                out.println("     * @return this builder for method chaining");
                out.println("     */");
            ...
            // Write build method
            out.println("    /**");
            out.println("     * Builds a new instance of {@link " + className + "}.");
            out.println("     * @return the new instance");
            out.println("     */");
        ...
        processingEnv.getMessager().printMessage(
            ...
            "Generated builder class: " + qualifiedBuilderName
        );
    }
    
    private Set<VariableElement> getFields(TypeElement typeElement) {
        Set<VariableElement> fields = new java.util.LinkedHashSet<>();
...
```java
// Example class using the @GenerateBuilder annotation
@GenerateBuilder
public class Person {
    ...
    // Default constructor needed for the builder
    public Person() {}
...
// Main class to demonstrate the generated builder
...
```bash
# Compile the annotation and processor
javac -proc:none GenerateBuilder.java BuilderProcessor.java

# Register the processor using META-INF/services
mkdir -p META-INF/services
echo "BuilderProcessor" > META-INF/services/javax.annotation.processing.Processor

# Compile the Person class with the processor
javac -cp . -processor BuilderProcessor Person.java

# Compile the demo class
javac -cp . AnnotationProcessorDemo.java

# Run the demo
java -cp . AnnotationProcessorDemo
```

## 10. Implementing a Custom ClassLoader

**Theory:**
- ClassLoaders are responsible for loading Java classes into the JVM
- The Java ClassLoader hierarchy consists of:
  1. Bootstrap ClassLoader (native code, loads core Java libraries)
  2. Extension ClassLoader (loads classes from JDK extensions directory)
  3. Application ClassLoader (loads classes from application classpath)
- Custom ClassLoaders allow for:
  1. Loading classes from non-standard locations (e.g., network, database)
  2. Modifying bytecode at load time
  3. Implementing custom class isolation
  4. Hot-swapping classes at runtime
- ClassLoaders follow the delegation model: child delegates to parent before attempting to load a class
...
public class CustomClassLoaderExample {
    public static void main(String[] args) {
        try {
            ...
            NetworkClassLoader networkLoader = new NetworkClassLoader("http://localhost:8080/classes/");
            FileSystemClassLoader fileLoader = new FileSystemClassLoader("/path/to/classes/");
            EncryptedClassLoader encryptedLoader = new EncryptedClassLoader("/path/to/encrypted/classes/", "secret-key");
            
            // Demonstrate loading a class from the network
            System.out.println("\n=== Loading class from network ===\n");
            try {
                // This would load RemoteClass from http://localhost:8080/classes/com/example/RemoteClass.class
                Class<?> remoteClass = networkLoader.loadClass("com.example.RemoteClass");
                Object instance = remoteClass.getDeclaredConstructor().newInstance();
                System.out.println("Successfully loaded and instantiated: " + instance);
            } catch (Exception e) {
                System.out.println("Could not load from network: " + e.getMessage());
                // For demo purposes, we'll continue with other examples
            }
            
            // Demonstrate loading a class from the file system
            System.out.println("\n=== Loading class from file system ===\n");
            try {
                // Create a simple test class for demonstration
                String classDir = System.getProperty("java.io.tmpdir");
                createSampleClass(classDir);
                
                // Create a new file system loader pointing to our temp directory
                FileSystemClassLoader tempLoader = new FileSystemClassLoader(classDir);
                
                // Load our dynamically created class
                Class<?> dynamicClass = tempLoader.loadClass("com.example.DynamicClass");
                Object instance = dynamicClass.getDeclaredConstructor().newInstance();
                System.out.println("Successfully loaded and instantiated: " + instance);
                
                // Call a method on the dynamically loaded class
                System.out.println("Invoking method: " + 
                    dynamicClass.getMethod("getMessage").invoke(instance));
                
                // Demonstrate class isolation between different class loaders
                System.out.println("\n=== Demonstrating class loader isolation ===\n");
                
                // Create a second loader pointing to the same directory
                FileSystemClassLoader tempLoader2 = new FileSystemClassLoader(classDir);
                Class<?> dynamicClass2 = tempLoader2.loadClass("com.example.DynamicClass");
                
                // Even though they're the same class from the same location,
                // they are loaded by different class loaders, so they are different classes
                System.out.println("Same class? " + (dynamicClass == dynamicClass2));
                System.out.println("Class1 loader: " + dynamicClass.getClassLoader());
                System.out.println("Class2 loader: " + dynamicClass2.getClassLoader());
                
                // But classes loaded by the same loader are the same
                Class<?> dynamicClass3 = tempLoader.loadClass("com.example.DynamicClass");
                System.out.println("Same loader, same class? " + (dynamicClass == dynamicClass3));
            } catch (Exception e) {
                System.out.println("Error in file system loading demo: " + e.getMessage());
                e.printStackTrace();
            }
            
            // Demonstrate the encrypted class loader
            System.out.println("\n=== Loading encrypted class ===\n");
            try {
                // In a real scenario, we would have encrypted class files
                // For this demo, we'll simulate it
                System.out.println("This would decrypt and load an encrypted class file");
                System.out.println("EncryptedClassLoader uses key: " + encryptedLoader.getEncryptionKey());
            } catch (Exception e) {
                System.out.println("Error in encrypted loading demo: " + e.getMessage());
            }
    ...
    // Helper method to create a sample class file for demonstration
    private static void createSampleClass(String directory) throws IOException {
        // This is a pre-compiled class file for a simple class:
        // package com.example;
        // public class DynamicClass {
        //     public String getMessage() { return "Hello from dynamically loaded class!"; }
        //     public String toString() { return "DynamicClass instance"; }
        // }
        
        // These are the actual bytes of the compiled class file
        byte[] classBytes = {
            -54, -2, -70, -66, 0, 0, 0, 52, 0, 23, 10, 0, 6, 0, 15, 9, 0, 16, 0, 17, 8, 0, 18, 10, 0, 19, 0, 20, 7, 0, 21, 7, 0, 22, 1, 0, 6, 60, 105, 110, 105, 116, 62, 1, 0, 3, 40, 41, 86, 1, 0, 4, 67, 111, 100, 101, 1, 0, 15, 76, 105, 110, 101, 78, 117, 109, 98, 101, 114, 84, 97, 98, 108, 101, 1, 0, 10, 103, 101, 116, 77, 101, 115, 115, 97, 103, 101, 1, 0, 20, 40, 41, 76, 106, 97, 118, 97, 47, 108, 97, 110, 103, 47, 83, 116, 114, 105, 110, 103, 59, 1, 0, 8, 116, 111, 83, 116, 114, 105, 110, 103, 1, 0, 10, 83, 111, 117, 114, 99, 101, 70, 105, 108, 101, 1, 0, 16, 68, 121, 110, 97, 109, 105, 99, 67, 108, 97, 115, 115, 46, 106, 97, 118, 97, 12, 0, 7, 0, 8, 1, 0, 34, 72, 101, 108, 108, 111, 32, 102, 114, 111, 109, 32, 100, 121, 110, 97, 109, 105, 99, 97, 108, 108, 121, 32, 108, 111, 97, 100, 101, 100, 32, 99, 108, 97, 115, 115, 33, 1, 0, 19, 68, 121, 110, 97, 109, 105, 99, 67, 108, 97, 115, 115, 32, 105, 110, 115, 116, 97, 110, 99, 101, 1, 0, 21, 99, 111, 109, 47, 101, 120, 97, 109, 112, 108, 101, 47, 68, 121, 110, 97, 109, 105, 99, 67, 108, 97, 115, 115, 1, 0, 16, 106, 97, 118, 97, 47, 108, 97, 110, 103, 47, 79, 98, 106, 101, 99, 116, 0, 33, 0, 5, 0, 6, 0, 0, 0, 0, 0, 3, 0, 1, 0, 7, 0, 8, 0, 1, 0, 9, 0, 0, 0, 29, 0, 1, 0, 1, 0, 0, 0, 5, 42, -73, 0, 1, -79, 0, 0, 0, 1, 0, 10, 0, 0, 0, 6, 0, 1, 0, 0, 0, 1, 0, 1, 0, 11, 0, 12, 0, 1, 0, 9, 0, 0, 0, 31, 0, 1, 0, 1, 0, 0, 0, 7, 18, 2, -80, 0, 0, 0, 1, 0, 10, 0, 0, 0, 6, 0, 1, 0, 0, 0, 2, 0, 1, 0, 13, 0, 12, 0, 1, 0, 9, 0, 0, 0, 31, 0, 1, 0, 1, 0, 0, 0, 7, 18, 3, -80, 0, 0, 0, 1, 0, 10, 0, 0, 0, 6, 0, 1, 0, 0, 0, 3, 0, 1, 0, 14, 0, 0, 0, 2, 0, 4
        };
        
        // Create the package directory structure
        File packageDir = new File(directory, "com/example");
        packageDir.mkdirs();
        
        // Write the class file
        File classFile = new File(packageDir, "DynamicClass.class");
        Files.write(classFile.toPath(), classBytes);
        
        System.out.println("Created sample class at: " + classFile.getAbsolutePath());
    }
}

// Custom ClassLoader that loads classes from a specified URL
class NetworkClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        try {
            // Convert class name to URL path (e.g., java.lang.String -> java/lang/String.class)
            ...
            throw new ClassNotFoundException("Could not load class " + name + ": " + e.getMessage(), e);
        }
    }
}
...
class FileSystemClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        try {
            ...
            File classFile = new File(baseDir + filePath);
...
// Custom ClassLoader that loads encrypted class files
class EncryptedClassLoader extends ClassLoader {
    private String baseDir;
    private String encryptionKey;
    private Map<String, Class<?>> classCache = new HashMap<>();
    
    public EncryptedClassLoader(String baseDir, String encryptionKey) {
        this.baseDir = baseDir.endsWith(File.separator) ? baseDir : baseDir + File.separator;
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        try {
            ...
            String filePath = name.replace('.', File.separatorChar) + ".encrypted";
            File encryptedFile = new File(baseDir + filePath);
            
            if (!encryptedFile.exists()) {
                throw new ClassNotFoundException("Encrypted class file not found: " + 
            ...
            // Decrypt the class bytes (simplified for example)
            byte[] classBytes = decrypt(encryptedBytes, encryptionKey);
            
            // Define the class from the decrypted byte array
    ...
    // Simplified decryption method (in a real implementation, this would use proper encryption)
    private byte[] decrypt(byte[] encryptedBytes, String key) {
        ...
        for (int i = 0; i < encryptedBytes.length; i++) {
            // Simple XOR with the key (NOT secure, just for demonstration)
            decrypted[i] = (byte) (encryptedBytes[i] ^ key.charAt(i % key.length()));
        }
...
1. **Custom ClassLoader Implementation**:
   - `NetworkClassLoader`: Loads classes from a remote URL
   - `FileSystemClassLoader`: Loads classes from a custom directory
   - `EncryptedClassLoader`: Loads and decrypts encrypted class files

2. **ClassLoader Delegation Model**:
   - Each custom loader extends the base `ClassLoader` class
   - They override `findClass()` but not `loadClass()`, preserving the delegation model

3. **Class Definition Process**:
   - Reading class bytes from various sources
   - Using `defineClass()` to convert raw bytes into a Java class

4. **Class Isolation**:
   - Demonstrating how the same class loaded by different loaders creates distinct Class objects
   - Showing how class identity is determined by both the class name and its loader

5. **Practical Applications**:
   - Loading classes from non-standard locations
   - Implementing class encryption for security
   - Creating class isolation environments
```